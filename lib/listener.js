  ...
  sock.ev.on("messages.upsert", async ({ type, messages }) => {
    try {
      ...
      for (const message of messages) {
        ...
        if(process.env.PROJECT_TYPE === "dev" || process.env.FULL_LOGS == "true"){
          console.log(JSON.stringify(message, null, 2));
        }
        const botId = sock.user.lid.split(":")[0] + "@lid";
        const botJId = sock.user.id.split(":")[0] + "@s.whatsapp.net";
        const fromMe = botId === message.key.participant
        ...
        const wcgProcessed = await Functions.wcgIntegration.processMessage(
          ...
          message?.key?.participantPn ||fromMe ? botJId : message.key.remoteJid,
          message?.pushName || "player",
          m.convo,
          m
        );
        if (message.key.fromMe && m.isPrivate() && await m.isSophia()) {
        ...
        // For Groups
          ...
          isGroup(message) && await m.isSophia() && (message.key?.fromMe || fromMe)) {
          if (
