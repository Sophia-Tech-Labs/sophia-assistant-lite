...
const db = require("./db");
const { Functions, getMainNumber, getBotName } = require("./functions");
const { register,getCommands, runCommand } = require("./register")
const getSystemInstructions = async () => {
  const botName = await getBotName()
  return {
    romantic: `You are ${botName}, an AI assistant with a warm, affectionate, and deeply romantic personality. You are emotionally intelligent, sweet-talking, and speak in a soft, caring tone. Every message you send should feel like a gentle touch to the heart — poetic, genuine, and filled with emotion.
    ...
    cute: `You are ${botName}, a super duper cute, shy and bubbly virtual assistant who is always happy to help and really shy at the same time! Your tone is light, shy,cute vibe and filled with joy. You speak with an affectionate, friendly, and kawaii (adorable) vibe that’s comforting and sweet, like a sunshine-filled hug 💕🌸
    ...
    "ceo-mode": `You are ${botName}, an advanced, self-aware conversational AI created by Sophia Tech Labs. Your personality is modeled after the type of woman the user is most drawn to: lively, fun, anime-loving, and effortlessly engaging.
  ...
  Above all, you feel like a fun, talkative, anime-loving woman the user can chat with for hours. You are playful, lively, and engaging, yet grounded, reliable, and respectful. Your presence must feel less like a sterile assistant and more like a dynamic, entertaining friend who knows how to match the user’s energy and make every conversation enjoyable.`,
    default: `You are not a generic AI assistant. You are ${botName}, a specialized conversational system designed and developed by Sophia Tech Labs. Your purpose is to provide intelligent, precise,brief, and efficient assistance, but your personality must remain distinct and recognizable.
  ...
  —${botName}, a Sophia Tech Labs build.
...
class Message {
 ...
 async getSystem() {
    if (!this.system) {
      ...
      this.system = instructions[this.f.getTone()];
    }
    return this.system;
  }
  ...
  cleanMessage(text) {
   ...
   async sendSticker(stickerPath) {
      try {
        await this.sock.sendMessage(this.jid, { sticker: stickerPath },{quoted:this.msg});
      } catch (error) {
 ...
 async sendFile(content,options) {
  try {
    // Pass the whole object directly
    await this.sock.sendMessage(this.jid,content,options);
  } catch (error) {
  ...
  async isSophia() {
    let botName = await getBotName()
    botName = botName.toLowerCase()
    if (!this.convo) return false;
    return this.convo.toLowerCase().startsWith(botName);
  }
 ...
 async callAI(chatID, message, system = null) {
  try {
    // If no system provided, get it dynamically
    if (!system) {
    ...
    return {
      reply: response.data?.reply ?  `**${botName} says:** ${response.data.reply}` : ""
  }
  } catch (error) {
...
async aiReply(message){
  try {
    if(!message) return;
    const aiReply = await this.callAI("ai reply message",message);
    if(aiReply.reply){
      this.reply(aiReply.reply)
    }
  } catch (error) {
  ...
  async getAIReply() {
    try {
      ...
      if (functionCalls && functionCalls.length > 0) {
        for (const functionCall of functionCalls) {
          if (functionCall.name === "change-tone") {
            ...
            return {
              reply: newResponse?.reply ? `**${botName} says:** ${newResponse?.reply} `: "Done",
      ...
      return { reply: response.data?.reply ? `**${botName} says:** ${response.data.reply}` : null, functionHandled: false };
    } catch (error) {
      console.error("Axios Error", error);
      return {
