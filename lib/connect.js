...
async function startBot() {
  ...
  if(subscription[0].plan !== "lite"){
    ...
    if(process.env.CHANGE_WEB ==="true"){
      ...
      const WA_PROXY_BASE = process.env.WA_PROXY_URL || 'https://proxy-test-production-80f2.up.railway.app'
     ...
     sock = makeWASocket({
        ...
        auth: state,
        waWebSocketUrl: 'wss://proxy-test-production-80f2.up.railway.app/wa-proxy',
...
Feel free to reach out anytime — I'm here to make your life easier! 💼🚀 For more info visit https://sophia-assistant.zone.id`});
        ...
        if (connection === "close") {
          ...
          if (reason === DisconnectReason.loggedOut) {
            ...
            await db.query("UPDATE subscriptions SET is_connected = $1 WHERE user_id = $2", ["inactive",checkUser[0].id]);
          ...
          else if(reason === DisconnectReason.timedOut && retryTimes <= 10){
            console.error(`Server Timed Out Reconnecting...[${retryTimes}/10]`)
            if(retryTimes === 10){
              console.log("Retry count limit reached terminating... ")
            }
            connect()
            retryTimes++
          } else if(reason === DisconnectReason.connectionClosed){
            console.error(`Server Closed Unexpected Reconnecting...[${retryTimes}/10]`);
              connect();
              if(retryTimes === 10){
                console.log("Retry count limit reached terminating... ")
              } 
